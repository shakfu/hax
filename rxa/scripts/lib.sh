...
# Environment first, then the key file. The key is never echoed, and goes to rxa through the
# environment rather than a flag so it stays out of the process table.
resolve_key() {
    local from_env="${!KEY_ENV:-}"
    if [ -n "$from_env" ]; then
        RXA_API_KEY="$from_env"
        KEY_SOURCE="\$$KEY_ENV"
    ...
    [ -n "$RXA_API_KEY" ] || die "the key from $KEY_SOURCE is empty"
    export RXA_API_KEY
}

setup_sandbox() {
    SANDBOX=$(mktemp -d "${TMPDIR:-/tmp}/rxa-live-XXXXXX") || die "could not make a sandbox"
